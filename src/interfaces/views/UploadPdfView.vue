<template>
  <div class="upload-container">
    <h1 class="title">UPLOAD DE PDF DAS/DARF</h1>
    <p class="subtitle">UPLOAD PDF DAS/DARF</p>

    <!-- Nova seção para download do arquivo de exemplo -->
    <div class="example-section">
      <h3 class="example-title">EXEMPLO DAS/DARF</h3>
      <p class="example-description">Baixe nosso arquivo de exemplo para ver o formato correto ou usar de teste.</p>
      <button @click="downloadExampleFile" class="example-download-button">
        <svg class="download-icon" viewBox="0 0 24 24">
          <path d="M5,20H19V18H5M19,9H15V3H9V9H5L12,16L19,9Z" />
        </svg>
        BAIXAR EXEMPLO DAS/DARF
      </button>
    </div>
...
const downloadExampleFile = async () => {
  try {
    ...
    const filePath = '/files/exemplo_das_darf.pdf'
    ...
    link.download = 'exemplo_das_darf.pdf'
    ...
    uploadResult.value = { 
      message: 'EXEMPLO BAIXADO COM SUCESSO! VERIFIQUE SEU COMPUTADOR.' 
    }
    
  } catch (error) {
    console.error('Erro ao baixar arquivo de exemplo:', error)
    uploadResult.value = { 
      message: 'ERRO AO BAIXAR EXEMPLO. TENTE NOVAMENTE.' 
    }
  } finally {
...
/* Estilos para a seção de exemplo */
.example-section {
...
.example-download-button:disabled {
  background: #444;
  cursor: not-allowed;
  transform: none;
}
